...
ORS_API_KEY = "5b3ce3597851110001cf6248efd9e06c79fe4e65b8ffabc5b7649d20"
ORS_URL = 'https://api.openrouteservice.org/v2/directions/driving-car'
ORS_NEAREST_URL = "https://api.openrouteservice.org/v2/nearest/point"
...
def find_route():
    try:
        ...
        preference = data.get('preference', 'fastest')  # Default to 'fastest'
        ...
        if 'features' in route_data and route_data['features']:
            route_geojson = route_data['features'][0]['geometry']
            return jsonify(route_geojson)  # Return geometry for frontend rendering
        else:
            print(f"No features found in the response")
...
#     # Geocode the source and destination location names to get coordinates
#     source_coords = geocode_location(source)
#     destination_coords = geocode_location(destination)
...
#     # Get nearest road points for both source and destination
#     source_nearest = get_nearest_road_point(source_coords) or source_coords  # Fallback to original coordinates
#     destination_nearest = get_nearest_road_point(destination_coords) or destination_coords  # Fallback
...
#         response.raise_for_status()  # This will raise an exception for HTTP error responses (e.g. 404, 500)
